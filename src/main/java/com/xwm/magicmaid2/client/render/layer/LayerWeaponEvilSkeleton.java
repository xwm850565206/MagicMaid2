...
public class LayerWeaponEvilSkeleton extends GeoLayerRenderer<EntityEvilSkeleton> {

    protected AnimatedGeoModel modelProvider = new ModelScythe();
    protected IGeoRenderer renderer;
    protected int tick;
    protected ItemStack itemStack = new ItemStack(ItemInit.SCYTHE);
    protected FloatBuffer MODELVIEW = GLAllocation.createDirectFloatBuffer(16);

    public LayerWeaponEvilSkeleton(IGeoRenderer<EntityEvilSkeleton> entityRendererIn) {
    ...
    @Override
    public void render(EntityEvilSkeleton entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        apply(anchor);
        GlStateManager.translate(4/16.0, 12/16.0, -1/16.0);
        GlStateManager.rotate(-120, 1, 0, 0);


        Minecraft.getMinecraft().getItemRenderer().renderItem(entitylivingbaseIn, itemStack, ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND);
