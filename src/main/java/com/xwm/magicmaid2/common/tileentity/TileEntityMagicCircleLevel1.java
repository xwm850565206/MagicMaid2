...
import com.xwm.magicmaid2.common.block.BlockMagicCircleLevel1;
...
public class TileEntityMagicCircleLevel1 extends TileEntityMagicCircle
{
    ...
    public void update()
    {
        boolean flag = checkFormulaWithLevel(Lists.newArrayList(
                inventory.get(0),
                inventory.get(1),
                inventory.get(2),
                inventory.get(3)), this.inventory.get(4));
        boolean flag1 = world.getBlockState(pos).getProperties().getOrDefault(BlockMagicCircleLevel1.OPEN, false).equals(true);
...
//            if (!world.isRemote)
        ...
        if (flag1 != this.isCooking()) {
            BlockMagicCircleLevel1.setState(this.isCooking(), world, pos);
            flag3 = true;
        }
    ...
    public boolean hasSlotForCook() {
        ...
        for (int i = 5; i < 8; i++)
        {
            ...
            {
                if ((itemstacks.size() + 5 > i
                        && this.getStackInSlot(i).isItemEqual(itemstacks.get(i - 5)))
                        && this.getStackInSlot(i).getMaxStackSize() > (this.getStackInSlot(i).getCount() + itemstacks.get(i-5).getCount()))
                    continue;
                else
                    return false;
            }
    ...
    public boolean checkFormulaWithLevel(List<ItemStack> raw, ItemStack input)
    {
        Formula formula = Formula.create(input, raw);
        Formula formula1 = MagicFormulaRegistry.getFormula(input);
        if (formula1.getLevel() > this.level)
            return false; // 10.1版本，添加石板等级，石板只能做出不高出自己等级的公式
        if (formula1 == Formula.EMPTY)
            return false;
        else
            return formula1.equals(formula);
    }
