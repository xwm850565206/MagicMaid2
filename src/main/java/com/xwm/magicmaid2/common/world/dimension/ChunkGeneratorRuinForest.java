...
import com.xwm.magicmaid2.common.world.gen.MapGenForestStructure;
import com.xwm.magicmaid2.common.world.gen.MapGenRuinForest;
...
public class ChunkGeneratorRuinForest implements IChunkGenerator
{
    /** RNG. */
    private final Random rand;
    protected static final IBlockState FACE = Blocks.GRASS.getDefaultState();
    ...
    private MapGenRuinForest forest = new MapGenRuinForest(this);
    private MapGenForestStructure forestStructure = new MapGenForestStructure(this);
    ...
    double[] ar;
    double[] br;
    private WorldGenMinable ironGenerator = new WorldGenMinable(Blocks.IRON_ORE.getDefaultState(), 8);
    private WorldGenMinable goldGenerator = new WorldGenMinable(Blocks.GOLD_ORE.getDefaultState(), 8);
    ...
    public ChunkGeneratorRuinForest(World p_i47241_1_, boolean p_i47241_2_, long p_i47241_3_, BlockPos p_i47241_5_)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            for (int j = 0; j < 16; ++j)
            {
                ...
                if (rand.nextFloat() < 0.025)
                    ...
                    primer.setBlockState(i, 50, j, iblockstate1);

                for (int k = 40; k < 50; k++)
                {
                    if (rand.nextFloat() < 0.25)
                        primer.setBlockState(i, k, j, iblockstate);
                    else
                        primer.setBlockState(i, k, j, iblockstate1);
                }
            }
        }
    }
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        if (Math.abs(x-5) < 10  && Math.abs(z-5) < 10) {
            ...
            {
                this.forest.generate(this.world, x, z, chunkprimer);
                this.forestStructure.generate(this.world, x, z, chunkprimer);
            }
        }
        ...
        {
            abyte[i] = (byte) Biome.getIdForBiome(BiomeInit.FOG_FOREST);
        }
    ...
    public void populate(int x, int z)
    {
        if (Math.abs(x-5) < 10  && Math.abs(z-5) < 10) {
            ...
            if (this.mapFeaturesEnabled)
            {
                this.forest.generateStructure(this.world, this.rand, new ChunkPos(x, z));
                this.forestStructure.generateStructure(this.world, this.rand, new ChunkPos(x, z));
            }
                ...
                if (this.getIslandHeightValue(x, z, 1, 1) > 40.0F)
                {
                    ...
                    for (int k = 0; k < j; ++k)
                    {
                        ...
                        if (j1 > 0)
                        {
                            ...
                            if (this.world.isAirBlock(blockpos.add(l, k1 + 1, i1)) && this.world.getBlockState(blockpos.add(l, k1, i1)).getBlock() == Blocks.GRASS)
                            {
    ...
    {
        if (structureName.equals("RuinForest"))
            return new BlockPos(78, 51, 54);

        if (structureName.equals("RuinForestStructure"))
            return new BlockPos(102, 51, 156);
    ...
    {
        if (structureName.equals("RuinForest"))
            return new AxisAlignedBB(57, 51, 52, 96, 51, 92).contains(new Vec3d(pos));
        if (structureName.equals("RuinForestStructure"))
            return new AxisAlignedBB(102, 51, 156, 130, 60, 190).contains(new Vec3d(pos));
        return false;
    }
